CLASS zcl_mm_material_document DEFINITION
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS get_instance
      ...
      RETURNING VALUE(output) TYPE REF TO zcl_mm_material_document
      RAISING   cx_no_entry_in_table.

    CLASS-METHODS wait_until_doc_in_db
      IMPORTING mat_doc_key TYPE mat_doc_key_dict
                wait_sec    TYPE i DEFAULT max_wait_sec
...
CLASS zcl_mm_material_document IMPLEMENTATION.
  METHOD get_instance.
    ...
    ASSIGN zcl_mm_material_document=>multitons[
                                             KEY primary_key COMPONENTS key = mat_doc_key ]
  ...
  METHOD wait_until_doc_in_db.
    ...
    RAISE EXCEPTION NEW zcx_bc_commit_wait_expire( textid  = zcx_bc_commit_wait_expire=>table_expired
  ...
  METHOD constructor.
    ...
    CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
      EXPORTING input  = mat_doc_key-mblnr
      IMPORTING output = mblnr.

    SELECT SINGLE mblnr, mjahr, bldat, budat
           FROM mkpf
           WHERE mblnr = @mblnr
             AND mjahr = @mat_doc_key-mjahr
           INTO @DATA(mkpf).

    IF sy-subrc <> 0.
      RAISE EXCEPTION NEW cx_no_entry_in_table( table_name = CONV #( table-header )
